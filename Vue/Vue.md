...
1. **立即调用函数**：当你在组件的*setup*函数中声明并立即调用一个函数的时，这个函数会在组件的初始化立即执行。这意味着他会在组件的其他生命周期钩子之前执行，甚至在模板和DOM被渲染之前。这对于初始化或者状态不依赖于DOM的逻辑是有用的。然而，由于此时组件尚未挂载，所以无法访问到dom元素，同时也无法保证所有的子组件或者异步数据已加载完成。
...
最基本的**数据绑定形式是文本插值**，使用‘**Mustache**’语法（双大括号）：
...
组件允许我们将UI划分为独立的、可重用的部分，并且可以对每个部分进行单独的思考。在实际应用中常常被组织成层层嵌套的树状结构。

### 定义一个组件

当使用构建步骤时，我们一般会将Vue组件定义在一个单独的.vue文件中，被称为单文件组件SFC：
